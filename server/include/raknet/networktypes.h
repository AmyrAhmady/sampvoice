...
struct PlayerID {
	...
	bool operator<(const PlayerID& obj) const {
		return (this->binaryAddress < obj.binaryAddress || (this->binaryAddress == obj.binaryAddress && this->port < obj.port));
	}

};

constexpr PlayerID UNASSIGNED_PLAYER_ID =
{
	0xFFFFFFFF, 0xFFFF
};
...
public:
	...
	virtual bool RPC(int* uniqueID, const char* data, unsigned int bitLength, PacketPriority priority, PacketReliability reliability, char orderingChannel, PlayerID playerId, bool broadcast, bool shiftTimestamp, NetworkID networkID, BitStream* replyFromTarget) = 0;
	virtual bool RPC(int* uniqueID, BitStream* bitStream, PacketPriority priority, PacketReliability reliability, char orderingChannel, PlayerID playerId, bool broadcast, bool shiftTimestamp, NetworkID networkID, BitStream* replyFromTarget) = 0;
